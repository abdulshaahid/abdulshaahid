...
export function Hero() {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (titleRef.current) {
        titleRef.current.style.transform = `perspective(1000px) rotateX(${-rotateX * 0.1}deg) rotateY(${rotateY * 0.1}deg)`
      }
      
      if (imageRef.current) {
        imageRef.current.style.transform = `perspective(1000px) rotateX(${5 - rotateX * 0.3}deg) rotateY(${-5 + rotateY * 0.3}deg) translateZ(30px) scale(1.02)`
      }
    }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-10 w-2 h-2 bg-white/20 rounded-full animate-float-1"></div>
        <div className="absolute top-40 right-20 w-1 h-1 bg-white/30 rounded-full animate-float-2"></div>
        <div className="absolute bottom-32 left-20 w-1.5 h-1.5 bg-white/25 rounded-full animate-float-3"></div>
        <div className="absolute top-60 left-1/3 w-1 h-1 bg-white/20 rounded-full animate-float-4"></div>
        <div className="absolute bottom-40 right-1/4 w-2 h-2 bg-white/15 rounded-full animate-float-5"></div>
        
        {/* Gradient orbs */}
        <div className="absolute top-1/4 -left-20 w-40 h-40 bg-white/5 rounded-full blur-3xl animate-pulse-slow"></div>
        <div className="absolute bottom-1/4 -right-20 w-60 h-60 bg-white/3 rounded-full blur-3xl animate-pulse-slower"></div>
      </div>
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid items-center gap-8 sm:gap-12 lg:gap-16 lg:grid-cols-2">
          ...
          <div className="order-2 lg:order-1 text-center lg:text-left space-y-6 sm:space-y-8">
            ...
            <div className="opacity-0 animate-slide-up" style={{animationDelay: '0.5s'}}>
              <div className="relative">
                <p className="text-lg sm:text-xl lg:text-2xl text-zinc-400 font-medium">
                  <span className="relative inline-block hover:text-white transition-colors duration-500 cursor-default">
            ...
            <div className="opacity-0 animate-slide-up" style={{animationDelay: '0.7s'}}>
              <p className="text-base sm:text-lg text-zinc-400 max-w-2xl mx-auto lg:mx-0 leading-relaxed hover:text-zinc-300 transition-colors duration-500">
                Crafting <span className="text-white font-semibold">sleek</span>, <span className="text-white font-semibold">scalable</span>, and <span className="text-white font-semibold">user-focused</span> web experiences with modern technologies.
              </p>
            </div>
            ...
            <div className="opacity-0 animate-slide-up" style={{animationDelay: '0.9s'}}>
              <div className="flex flex-col sm:flex-row gap-4 items-center justify-center lg:justify-start">
                <Button asChild className="w-full sm:w-auto rounded-2xl bg-white text-black hover:bg-zinc-200 px-8 py-3 text-base font-semibold transform hover:scale-105 hover:-translate-y-1 transition-all duration-300 shadow-lg hover:shadow-2xl hover:shadow-white/25 group relative overflow-hidden">
                  <a href="#portfolio">
                ...
                <Button
                  asChild
                  variant="outline"
                  className="w-full sm:w-auto rounded-2xl border-white/30 text-white hover:bg-white/10 bg-transparent px-8 py-3 text-base font-semibold transform hover:scale-105 hover:-translate-y-1 transition-all duration-300 hover:border-white/50 hover:shadow-lg hover:shadow-white/20 group relative overflow-hidden backdrop-blur-sm"
                >
                  <a href="#contact">
                    <span className="relative z-10 flex items-center gap-2">
                      Contact Me
          ...
          {/* Image Section */}
          <div className="order-1 lg:order-2 opacity-0 animate-slide-up" style={{animationDelay: '1.1s'}}>
            <div className="relative mx-auto w-64 h-64 sm:w-80 sm:h-80 lg:w-96 lg:h-96">
              {/* Outer rotating rings */}
              <div className="absolute inset-0 rounded-full">
                <div className="absolute inset-0 rounded-full border-2 border-white/20 animate-spin-slow hover:border-white/40 transition-colors duration-500"></div>
                <div className="absolute inset-4 rounded-full border border-white/10 animate-spin-reverse hover:border-white/30 transition-colors duration-500"></div>
                <div className="absolute inset-8 rounded-full border border-white/5 animate-spin-slow-reverse hover:border-white/20 transition-colors duration-500"></div>
              </div>
              
              {/* Glowing orbs */}
              <div className="absolute -top-4 -right-4 w-8 h-8 bg-white/30 rounded-full blur-md animate-pulse-soft"></div>
              <div className="absolute -bottom-6 -left-6 w-6 h-6 bg-white/20 rounded-full blur-sm animate-pulse-offset"></div>
              ...
              >
                ...
                <div className="relative h-full w-full">
                  ...
                  {/* Shine effect */}
                  <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 group-hover:animate-shine"></div>
                </div>
              ...
              {/* Floating particles */}
              <div className="absolute top-12 -left-8 w-2 h-2 bg-white/40 rounded-full animate-float-particle-1"></div>
              <div className="absolute bottom-16 -right-12 w-1.5 h-1.5 bg-white/30 rounded-full animate-float-particle-2"></div>
              <div className="absolute top-1/3 -left-12 w-1 h-1 bg-white/50 rounded-full animate-float-particle-3"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        @keyframes float-particle-2 {
          ...
          50% { transform: translate(-15px, -25px) scale(1.5); opacity: 0.7; }
        }
        ...
        .animate-pulse-offset { animation: pulse-offset 3s ease-in-out infinite; animation-delay: 1s; }
